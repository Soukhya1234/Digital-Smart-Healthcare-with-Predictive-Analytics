...
router.post('/register', async (req, res) => {
  const { name, email, phoneNumber, address, age, password } = req.body;
  try {
    if (!name || !email || !phoneNumber || !address || !age || !password)
      return res.status(400).json({ message: 'All fields are required' });
    ...
    const user = new User({ name, email, phoneNumber, address, age, password: hashedPassword });
    await user.save();

    res.status(201).json({ user: { userId: user._id, name: user.name, email: user.email } });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
  }
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ user: { userId: user._id, name: user.name, email: user.email } });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
  }
...
router.post('/doctor-register', async (req, res) => {
  ...
  try {
    ...
    const doctor = new Doctor({ name, email, phoneNumber, password: hashedPassword, specialization });
    await doctor.save();

    res.status(201).json({
      doctor: { doctorId: doctor._id, name: doctor.name, email: doctor.email, specialization }
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
...
router.post('/doctor-login', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ doctor: { doctorId: doctor._id, name: doctor.name, email: doctor.email, specialization: doctor.specialization } });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
  }
